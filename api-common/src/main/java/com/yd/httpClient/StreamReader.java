...
import com.yd.common.util.stream.Filer;
...
public class StreamReader {
    ...
    public static String readString(InputStream is, String encoding) throws IOException {
        ...
        if (is != null) {
            ...
            logger.error("Read stream from inputStream: " + result);
            rd.close();
        }
        return result;
    }
    ...
    public static File readFile(InputStream is, int contentLength, File writeFile, FileRenamer renamer) throws IOException {
        ...
        if (is != null) {
            ...
            if (renamer == null && writeFile.exists()) {
                if (contentLength == 0) {
                    logger.error("File download was complete, don't download " + writeFile.getPath());
                    return writeFile;
                }
